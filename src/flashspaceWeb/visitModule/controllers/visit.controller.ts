...
export const getVisits = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch visits", error });
  }
};
...
export const getUserVisits = async (req: Request, res: Response) => {
  try {
    ...
    const visits = await Visit.find({
      $or: [{ user: userId }, { clientEmail: userEmail }],
    }).sort({ date: -1 });
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch user visits", error });
  }
};
...
export const createVisit = async (req: Request, res: Response) => {
  try {
    ...
    const newVisit = new Visit(userData);
    await newVisit.save();
    res.status(201).json({ success: true, data: newVisit });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Failed to create visit", error });
  }
};
