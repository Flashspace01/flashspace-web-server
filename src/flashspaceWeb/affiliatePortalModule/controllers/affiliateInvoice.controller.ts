...
export const getInvoices = async (req: Request, res: Response) => {
    try {
        ...
        const bookings = await BookingModel.find({
            ...
            isDeleted: false
        }).select("_id bookingNumber plan spaceSnapshot user createdAt status").lean();
        ...
        const mappedInvoices = bookings.map((b: any) => {
            ...
            if (inv) {
                ...
                return {
                    ...inv,
                    client: inv.billingAddress?.company || inv.billingAddress?.name || user.fullName || "Client",
                    clientAddress: [
                ...
                const commission = parseFloat((paidAmount * COMMISSION_RATE).toFixed(2));
                const mockInvNum = b.bookingNumber ? b.bookingNumber.replace("FS-", "INV-") : `INV-MOCK-${b._id.toString().slice(-6).toUpperCase()}`;
                ...
                return {
                    ...
                    commission: commission,
                    date: b.createdAt,
                    status: ["active", "completed"].includes(b.status) ? "paid" : "pending",
                    items: [
                        {
        ...
        const totalEarnings = mappedInvoices.reduce((sum, inv) => sum + inv.commission, 0);
...
export const getInvoiceById = async (req: Request, res: Response) => {
    try {
        ...
        let invoice = await InvoiceModel.findOne({ _id: id, isDeleted: false }).lean();
        let booking;
        ...
        if (!invoice) {
            ...
            const commission = parseFloat((paidAmount * COMMISSION_RATE).toFixed(2));
            const mockInvNum = booking.bookingNumber ? booking.bookingNumber.replace("FS-", "INV-") : `INV-MOCK-${booking._id.toString().slice(-6).toUpperCase()}`;
            ...
            const mappedInvoice = {
                ...
                status: ["active", "completed"].includes(booking.status) ? "paid" : "pending",
                items: [
                    {
        ...
        const mappedInvoice = {
            ...invoice,
            client: invoice.billingAddress?.company || invoice.billingAddress?.name || "Client",
            clientAddress: [
