...
export const getAvailability = async (req: Request, res: Response) => {
    try {
        const { days } = res.locals.validatedQuery || { days: parseInt(req.query.days as string) || 7 };
...
export const bookMeeting = async (req: Request, res: Response) => {
    try {
        const { fullName, email, phoneNumber, slotTime, notes } = res.locals.validatedBody || req.body;
