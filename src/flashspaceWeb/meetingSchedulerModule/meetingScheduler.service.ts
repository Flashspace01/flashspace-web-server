...
export class MeetingSchedulerService {
  ...
  private static generateDaySlots(
    ...
    while (currentSlot < dayEnd) {
      ...
      const isBusy = busyTimes.some((busy) => {
        const busyStart = DateTime.fromJSDate(busy.start).setZone(
          this.TIMEZONE,
        );
  ...
  static async bookMeeting(
    ...
    const slotTime = DateTime.fromJSDate(new Date(request.slotTime)).setZone(
      this.TIMEZONE,
    );
    ...
    if (
      slotTime.hour < this.WORKING_START_HOUR ||
      slotTime.hour >= this.WORKING_END_HOUR
    ) {
      return {
        success: false,
   ...
   */
  static async getMeetings(filter: any): Promise<any[]> {
