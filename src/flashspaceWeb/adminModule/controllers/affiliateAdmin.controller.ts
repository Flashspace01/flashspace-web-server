...
export const getAllAffiliates = async (req: Request, res: Response) => {
  try {
    ...
    const affiliates = await (UserModel as any)
      .find({
    ...
    const affiliateIdStrings: string[] = affiliates.map((a: any) =>
      a._id.toString(),
    );
    ...
    const coupons = await (CouponModel as any)
      .find({
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        totalRevenue: result.reduce(
          (sum: number, a: any) => sum + (a.totalRevenue || 0),
          0,
        ),
        totalCommissionPayable: result.reduce(
...
export const getAffiliateClients = async (req: Request, res: Response) => {
  try {
    ...
    const affiliateIdStr: string = Array.isArray(affiliateId)
      ? affiliateId[0]
      : affiliateId;
    ...
    const affiliate = await (UserModel as any)
      .findById(affiliateIdStr)
    ...
    const userIds = [
      ...new Set(
        (bookings as any[]).map((b: any) => b.user?.toString()).filter(Boolean),
      ),
    ];
    ...
    const coupon = await (CouponModel as any)
      .findOne({
    ...
    return res
      ...
      .json({
        success: false,
        message: "Failed to fetch affiliate clients",
        error: error.message,
      });
  }
};
