...
export const upsertSpaceUserKycBusinessInfo = async (req: Request, res: Response) => {
	try {
		...
		if (!companyName && !companyType && !industry && !gstNumber && !cinRegistrationNumber && !registeredAddress && !companyPartners) {
		...
		if (!kyc) {
			return res.status(404).json({
				success: false,
				message: "KYC record not found. Please submit personal information first.",
			});
		}
		...
		// Whenever business info changes, set overall status back to pending review
...
export const getAllSpacePartnerKyc = async (req, res) => {
	try {
		const kycs = await SpaceUserKycModel.find().populate('userId', 'fullName email phoneNumber');
		res.json({ success: true, data: kycs });
	} catch (err) {
		res.status(500).json({ success: false, message: 'Failed to fetch space partner KYC records', error: err.message });
	}
};

// Get a single space partner KYC record by its id (admin)
export const getSpacePartnerKycById = async (req, res) => {
	try {
		const { id } = req.params;
		const kyc = await SpaceUserKycModel.findById(id).populate('userId', 'fullName email phoneNumber');
		if (!kyc) {
			return res.status(404).json({ success: false, message: 'KYC record not found' });
		}
		res.json({ success: true, data: kyc });
	} catch (err) {
		res.status(500).json({ success: false, message: 'Failed to fetch KYC record', error: err.message });
	}
};
import { Request, Response } from "express";
import fs from "fs";
import path from "path";
import { SpaceUserKycModel, KycDecisionStatus } from "../models/spaceUserKyc.model";
...
export const upsertSpaceUserKyc = async (req: Request, res: Response) => {
	try {
		...
		// Whenever personal info changes, set overall status back to pending review
		kyc.overallStatus = "pending";
		kyc.overallRejectMessage = undefined;
...
export const reviewSpaceUserKycDocument = async (req: Request, res: Response) => {
	try {
...
export const reviewSpaceUserKycOverall = async (req: Request, res: Response) => {
	try {
		...
		kyc.overallRejectMessage = status === "rejected" ? rejectMessage : undefined;
