...
// Helper to parse price string to number ("₹1,083/month" -> 1083)
...
async function generatePropertyAndPartner(space: any) {
  const emailIdentifier = `${space.name}-${space.city}`
    .replace(/\s+/g, "")
    .replace(/[^a-zA-Z0-9]/g, "")
    .toLowerCase()
    .substring(0, 30);
  ...
  // City-specific baseline coordinates (Lng, Lat)
  const cityCoordinates: Record<string, [number, number]> = {
    ...
    Jammu: [74.857, 32.733],
    Dharamshala: [76.3234, 32.219],
  };

  const baseline = cityCoordinates[space.city] || [77.209, 28.6139];
  // Add some jitter to prevent overlapping markers (approx +/- 2-3km)
  const jitterLng = (Math.random() - 0.5) * 0.04;
  const jitterLat = (Math.random() - 0.5) * 0.04;
  const coordinates: [number, number] = [
    baseline[0] + jitterLng,
    baseline[1] + jitterLat,
  ];
  ...
  let property = await PropertyModel.findOne({ address: space.address });
  if (!property) {
    ...
    // Update location to fix the map issue for existing properties
    property.location = { type: "Point", coordinates };
    await property.save();
  }
...
async function seedDatabase() {
  try {
    ...
    await PropertyModel.deleteMany({
      name: {
        $in: [
          ...coworkingSpaces.map((s) => s.name),
          ...virtualOffices.map((s) => s.name),
        ],
      },
    });
    await UserModel.deleteMany({
      email: { $regex: /^partner-.*@flashspace\.ai$/ },
    });
    console.log("Existing generated data cleared!");

    console.log("\nSeeding Coworking Spaces...");
    for (const space of coworkingSpaces) {
      const { user, property } = await generatePropertyAndPartner(space);
      ...
      await CoworkingSpaceModel.create({
        ...
        capacity: 50,
        sponsored: false,
        popular: space.popular || (space.reviews >= 50 && space.rating > 4.5),
        approvalStatus: SpaceApprovalStatus.ACTIVE,
        partnerPricePerMonth: parsePrice(space.price),
        finalPricePerMonth: parsePrice(space.price),
        floors: [
          {
            floorNumber: 1,
            name: "Ground Floor",
            tables: [
              {
                tableNumber: "T1",
                seats: [
                  { seatNumber: "S1", isActive: true },
                  { seatNumber: "S2", isActive: true },
                ],
              },
            ],
          },
    ...
    for (const space of virtualOffices) {
      const { user, property } = await generatePropertyAndPartner(space);
      ...
      await VirtualOfficeModel.create({
        ...
        approvalStatus: SpaceApprovalStatus.ACTIVE,
        popular: space.popular || (space.reviews >= 50 && space.rating > 4.5),
        partnerGstPricePerYear: parsePrice(space.gstPlanPrice) * 12,
        finalGstPricePerYear: parsePrice(space.gstPlanPrice) * 12,
        partnerMailingPricePerYear: parsePrice(space.mailingPlanPrice) * 12,
        finalMailingPricePerYear: parsePrice(space.mailingPlanPrice) * 12,
        partnerBrPricePerYear: parsePrice(space.brPlanPrice) * 12,
        finalBrPricePerYear: parsePrice(space.brPlanPrice) * 12,
        avgRating: space.rating,
        totalReviews: space.reviews,
        amenities: space.features,
        images: [space.image],
    ...
    for (const space of coworkingSpaces.slice(0, 10)) {
      const { user, property } = await generatePropertyAndPartner(space);
      ...
      await MeetingRoomModel.create({
        ...
        partner: user._id,
        approvalStatus: SpaceApprovalStatus.ACTIVE,
        name: `${space.name} Board Room`,
        capacity: 12,
        type: "board_room",
        partnerPricePerHour: 800,
        finalPricePerHour: 800,
        partnerPricePerDay: 5000,
        finalPricePerDay: 5000,
        operatingHours: {
          openTime: "09:00",
          closeTime: "18:00",
          daysOpen: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"],
        },
        amenities: ["Projector", "Whiteboard", "High-Speed WiFi", "Coffee"],
        images: [space.image],
        avgRating: 4.7,
        totalReviews: 45,
        isActive: true,
    ...
    const totalCoworking = await CoworkingSpaceModel.countDocuments();
    const totalVirtualOffices = await VirtualOfficeModel.countDocuments();
    const totalMeetingRooms = await MeetingRoomModel.countDocuments();
    const totalProperties = await PropertyModel.countDocuments({
      address: { $in: coworkingSpaces.map((c) => c.address) },
    });
    const totalPartners = await UserModel.countDocuments({
      email: { $regex: /^partner-.*@flashspace\.ai$/ },
    });

    console.log("\n✅ Database seeded successfully!");
    console.log("\nSummary:");
    console.log(`- Partners Created: ${totalPartners}`);
    console.log(`- Properties Created: ${totalProperties}`);
    console.log(`- Coworking Spaces: ${totalCoworking}`);
    console.log(`- Virtual Offices: ${totalVirtualOffices}`);
    console.log(`- Meeting Rooms: ${totalMeetingRooms}`);

    console.log(
      "\nYou can log in with any partner email, e.g. partner-workzoneahmedabad@flashspace.ai with password 'Password123!'",
    );
