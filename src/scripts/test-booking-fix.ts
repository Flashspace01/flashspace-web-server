...
async function runTest() {
    try {
        ...
        const space = await VirtualOfficeModel.findOne({ isDeleted: { $ne: true } });
        if (!space) {
        ...
        if (!space.partner) {
            ...
            if (partnerUser) {
                await VirtualOfficeModel.findByIdAndUpdate(space._id, { partner: partnerUser._id });
                log(`Assigned partner ${partnerUser.email} to space.`);
            } else {
                log("No partner user found to assign. Using current user as partner.");
                await VirtualOfficeModel.findByIdAndUpdate(space._id, { partner: user._id });
            }
        }
        ...
        log(`Booking created successfully: ${booking._id} with partnerId: ${booking.partnerId}`);
        ...
        if (notification) {
            ...
            log("Notification NOT found in DB yet (it might be sent via Socket.IO but let's check NotificationModel if it stores it)");
        }
