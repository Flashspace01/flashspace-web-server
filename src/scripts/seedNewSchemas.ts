console.log("Starting new schema seed script...");
...
async function seedDatabase() {
    try {
        ...
        // Delete only the mock partner we create
        await UserModel.deleteOne({ email: "seedpartner@flashspace.com" });
        ...
        // Track created properties to avoid duplication via name/address
        const propertyMap: { [key: string]: mongoose.Types.ObjectId } = {};

        // First, consolidate all spaces to build Properties
        ...
        console.log(`\nCreated ${Object.keys(propertyMap).length} Properties!`);
        ...
        console.log("\n✅ Database seeded successfully with new schemas!");
        process.exit(0);
    } catch (error) {
        console.error("Error seeding database:", error);
        fs.writeFileSync("seedNewSchemas-error.log", `Error seeding: ${error}\n${error instanceof Error ? error.stack : ''}`);
        process.exit(1);
    }
}
