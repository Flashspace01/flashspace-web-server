...
async function testAuthentication() {
  ...
  if (authToken) {
    results.push(
      await runTest("Auth - Check Authentication", async () => {
  ...
  // 4. Forgot Password
  results.push(
    await runTest("Auth - Forgot Password", async () => {
  ...
  if (authToken) {
    results.push(
      await runTest("Auth - Logout All Sessions", async () => {
        const response = await api.post(
          "/auth/logout-all",
          {},
          {
  ...
  results.push(
    await runTest("Auth - Re-login", async () => {
      const response = await api.post("/auth/login", {
        email: TEST_EMAIL,
        password: TEST_PASSWORD,
      });
...
async function testContactForm() {
  ...
  results.push(
    await runTest("Contact Form - Create", async () => {
  ...
  results.push(
    await runTest("Contact Form - Get All", async () => {
  ...
  if (contactId) {
    results.push(
      await runTest("Contact Form - Get By ID", async () => {
        const response = await api.get(
          `/contactForm/getContactFormById/${contactId}`,
        );
  ...
  if (contactId) {
    results.push(
      await runTest("Contact Form - Update", async () => {
        const response = await api.put(
          `/contactForm/updateContactForm/${contactId}`,
          {
            isActive: false,
  ...
  if (contactId) {
    results.push(
      await runTest("Contact Form - Delete", async () => {
        const response = await api.delete(
          `/contactForm/deleteContactForm/${contactId}`,
        );
...
async function testVirtualOffice() {
  ...
  results.push(
    await runTest("Virtual Office - Get All", async () => {
      ...
      if (count > 0)
        console.log("   First office:", response.data.data[0].name);
      return response.data;
    }),
  );
  ...
  // 3. Get by City
  results.push(
    await runTest("Virtual Office - Get By City", async () => {
...
async function testCoworkingSpace() {
  ...
  results.push(
    await runTest("Coworking Space - Create", async () => {
      const response = await api.post(
  ...
  results.push(
    await runTest("Coworking Space - Get All", async () => {
  ...
  // 3. Get by City
  results.push(
    await runTest("Coworking Space - Get By City", async () => {
  ...
  if (coworkingSpaceId) {
    results.push(
      await runTest("Coworking Space - Get By ID", async () => {
        const response = await api.get(
          `/coworkingSpace/getById/${coworkingSpaceId}`,
        );
  ...
  if (coworkingSpaceId) {
    results.push(
      await runTest("Coworking Space - Update", async () => {
        const response = await api.put(
          `/coworkingSpace/update/${coworkingSpaceId}`,
          {
            price: "₹9,499/month",
            type: "Dedicated Desk",
          },
  ...
  if (coworkingSpaceId) {
    results.push(
      await runTest("Coworking Space - Delete", async () => {
        const response = await api.delete(
          `/coworkingSpace/delete/${coworkingSpaceId}`,
          {
            headers: { Authorization: `Bearer ${authToken}` },
          },
        );
...
async function testPartnerInquiry() {
  ...
  results.push(
    await runTest("Partner Inquiry - Submit", async () => {
  ...
  results.push(
    await runTest("Partner Inquiry - Get All", async () => {
  ...
  if (partnerInquiryId) {
    results.push(
      await runTest("Partner Inquiry - Update Status", async () => {
        const response = await api.put(
          `/partnerInquiry/${partnerInquiryId}/status`,
          {
            status: "contacted",
          },
        );
...
async function testDashboard() {
  ...
  results.push(
    await runTest("Dashboard - Get Overview", async () => {
      const response = await api.get("/user/dashboard", { headers });
      console.log("   Dashboard data received:", response.data.success);
      if (response.data.data) {
        console.log(
          "   Active Services:",
          response.data.data.activeServices || 0,
        );
        console.log(
          "   KYC Status:",
          response.data.data.kycStatus || "Not set",
        );
      }
      return response.data;
    }),
  ...
  // 2. Get KYC Status
  results.push(
    await runTest("Dashboard - Get KYC Status", async () => {
      const response = await api.get("/user/kyc", { headers });
      console.log(
        "   KYC Status:",
        response.data.data?.overallStatus || "Not set",
      );
      return response.data;
    }),
  );
  ...
  results.push(
    await runTest("Dashboard - Update Business Info", async () => {
      const response = await api.put(
        "/user/kyc/business-info",
        {
  ...
  // 5. Get All Bookings
  results.push(
    await runTest("Dashboard - Get Bookings", async () => {
  ...
  if (bookingId) {
    results.push(
      await runTest("Dashboard - Get Booking By ID", async () => {
  ...
  if (bookingId) {
    results.push(
      await runTest("Dashboard - Toggle Auto-Renew", async () => {
        const response = await api.patch(
          `/user/bookings/${bookingId}/auto-renew`,
          {
            autoRenew: false,
          },
          { headers },
  ...
  // 8. Get All Invoices
  results.push(
    await runTest("Dashboard - Get Invoices", async () => {
  ...
  if (invoiceId) {
    results.push(
      await runTest("Dashboard - Get Invoice By ID", async () => {
  ...
  results.push(
    await runTest("Dashboard - Get Support Tickets", async () => {
  ...
  results.push(
    await runTest("Dashboard - Create Support Ticket", async () => {
      const response = await api.post(
  ...
  if (ticketId) {
    results.push(
      await runTest("Dashboard - Get Ticket By ID", async () => {
  ...
  if (ticketId) {
    results.push(
      await runTest("Dashboard - Reply to Ticket", async () => {
        const response = await api.post(
          `/user/support/tickets/${ticketId}/reply`,
          {
            message:
              "Thank you for looking into this issue. This is a test reply.",
          },
          { headers },
        );
...
async function testSpacePortal() {
  ...
  results.push(
    await runTest("Space Portal - Get All Spaces", async () => {
      ...
      console.log("   Total spaces for partner:", total);

      // Pick first space ID from any available category
      const data = response.data.data;
      const firstSpace =
        data.coworkingSpaces?.[0] ||
        data.virtualOffices?.[0] ||
        data.meetingRooms?.[0];
      if (firstSpace) {
        portalSpaceId = firstSpace._id;
        console.log("   Captured Space ID:", portalSpaceId);
      }
      return response.data;
    }),
  );
  ...
  if (portalSpaceId) {
    results.push(
      await runTest("Space Portal - Get Space By ID", async () => {
        const response = await api.get(`/spaceportal/spaces/${portalSpaceId}`, {
          headers,
        });
        console.log("   Space found:", !!response.data.data);
        return response.data;
      }),
    );
  }
  ...
  // 3. Get All Tickets
  results.push(
    await runTest("Space Portal - Get All Tickets", async () => {
      const response = await api.get("/spaceportal/tickets", { headers });
      const tickets = response.data.data?.tickets || [];
      const total = response.data.data?.pagination?.total || 0;
      console.log("   Total tickets for partner:", total);
      if (tickets.length > 0) {
        // NOTE: The service's mapTicketToUi returns ticketNumber as 'id'
        // But getTicketById service expects MongoDB ObjectId.
        // For now, let's try to find if it has an internal _id or if the UI uses ticketNumber differently.
        // If the service is strictly ObjectId, we might need to fix it.
        portalTicketId = tickets[0].id;
        console.log("   Captured Ticket ID/Number:", portalTicketId);
      }
      return response.data;
    }),
  );
  ...
  if (portalTicketId) {
    results.push(
      await runTest("Space Portal - Get Ticket By ID", async () => {
        const response = await api.get(
          `/spaceportal/tickets/${portalTicketId}`,
          { headers },
        );
        console.log("   Ticket found:", !!response.data.data);
        return response.data;
      }),
    );
  }
  ...
  if (portalTicketId) {
    results.push(
      await runTest("Space Portal - Update Ticket Status", async () => {
        const response = await api.patch(
          `/spaceportal/tickets/${portalTicketId}`,
          {
            status: "in-progress",
        ...
        console.log("   Status updated:", response.data.success);
        return response.data;
      }),
    );
  }
...
async function testAdditionalEndpoints() {
  ...
  results.push(
    await runTest("Auth - Verify Email (Legacy)", async () => {
      try {
  ...
  results.push(
    await runTest("Auth - Google OAuth", async () => {
      try {
  ...
  results.push(
    await runTest("Auth - Reset Password", async () => {
      try {
...
async function main() {
  ...
  try {
    ...
    allResults.push(...(await testSpacePortal()));
...
function printTestSummary(results: TestResult[]) {
  ...
  const categories = {
    ...
    "Coworking Space": results.filter((r) =>
    ...
    "Space Portal": results.filter((r) => r.name.includes("Space Portal")),
  ...
  for (const [category, tests] of Object.entries(categories)) {
    if (tests.length > 0) {
      ...
      const rate =
        totalInCat > 0 ? Math.round((passedInCat / totalInCat) * 100) : 0;
