    ...
    await runTest("Coworking Space - Create", async () => {
      const response = await api.post(
        "/coworkingSpace/create",
        {
          ...
          amenities: ["WiFi", "Coffee", "Pantry"],
          popular: true,
          location: {
            type: "Point",
            coordinates: [72.8656, 19.0596],
          },
          image:
            "https://images.unsplash.com/photo-1497366754035-f200968a6e72?w=800&auto=format&fit=crop",
        },
      ...
      coworkingSpaceId = response.data.data?._id || response.data.data?.id;
    ...
    results.push(
      await runTest("Coworking Space - Update", async () => {
        const response = await api.put(
          ...
          {
            name: "Test Coworking Space Updated",
            type: "Dedicated Desk",
          },
          {
            headers: { Authorization: `Bearer ${authToken}` },
